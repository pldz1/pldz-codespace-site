...
from plugins.mongodb.authenticated import AuthorizedHandler, SECRET_KEY, ALGORITHM
...
async def login(data: LoginData):
    ...
    resp = JSONResponse({'data': {"flag": True, "isAdmin": is_admin, 'log': '登录成功'}})
...
async def register(data: LoginData):
    ...
    # 存储新用户
    AuthorizedHandler.add_user(data.username, AuthorizedHandler.hash_password(data.password))
    access = AuthorizedHandler.create_access_token(data.username)
    refresh = AuthorizedHandler.create_refresh_token(data.username)
...
async def refresh(request: Request):
    ...
    resp = JSONResponse({'data': {'flag': True, 'username': username, 'isAdmin': is_admin, 'log': '刷新成功'}})
