...
async function onCheckImageName() {
  ...
  checkNameTimer = setTimeout(async () => {
    ...
    const res = await checkImageExit(props.articleMeta.category, imageName.value);
    if (!res) {
...
async function onUploadImage() {
  ...
  const res = await uploadArticleImage(props.articleMeta.category, imageName.value);
  // 检查返回结果
...
async function onUploadImageFromCopy() {
  ...
  const res = await uploadArticleImageFromCopy(props.articleMeta.category, imageName.value);
  // 检查返回结果
  afterUploadImage(res);
}
</script>
